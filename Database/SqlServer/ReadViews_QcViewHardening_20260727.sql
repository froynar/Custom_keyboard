:ON ERROR EXIT
-- Immutable final definitions for schema version 2026.07.27-qc-view-hardening.
-- Do not modify this snapshot after the migration has been released.
    ...
    SELECT
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY
                ...
                session_summary.completed_at DESC,
...
INNER JOIN dbo.brands AS brand_row
    ON brand_row.id = kit_row.brand_id
...
INNER JOIN dbo.brands AS brand_row
    ON brand_row.id = switch_row.brand_id
...
INNER JOIN dbo.brands AS brand_row
    ON brand_row.id = keycap_row.brand_id
...
INNER JOIN dbo.brands AS brand_row
    ON brand_row.id = stabilizer_row.brand_id
...
SELECT
    ...
    latest_qc.completed_at AS qc_completed_at,
...
INNER JOIN dbo.builds AS build_row
    ON build_row.id = request_row.build_id
INNER JOIN dbo.users AS buyer_user
    ON buyer_user.id = build_row.buyer_id
INNER JOIN dbo.keyboard_kits AS kit_row
    ON kit_row.id = build_row.kit_id
LEFT JOIN dbo.users AS seller_user
    ON seller_user.id = request_row.seller_user_id
LEFT JOIN dbo.seller_profiles AS seller_profile
    ON seller_profile.user_id = request_row.seller_user_id
